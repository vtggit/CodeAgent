...
class Worker:
    ...
    async def _post_results_to_github(
        ...
        try:
            ...
            self._logger.warning(
                "Failed to post results to GitHub issue #%d: %s",
                issue_number,
                str(e),
            )

    def _format_github_comment(self, result: DeliberationResult) -> str:
        """
        Format deliberation results as a GitHub issue comment.

        Args:
            result: The deliberation result to format
        ...
        lines = [
            ...
            "",
            f"**Status:** {result.workflow.status.value}",
        ...
        if result.agent_participation:
            ...
            for agent, count in sorted(
                ...
                lines.append(f"- **{agent}**: {count} comment(s)")
            lines.append("")
        ...
        lines.append("---")
        lines.append(
            "*Generated by Multi-Agent GitHub Issue Routing System*"
        )
